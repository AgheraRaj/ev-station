...
function getStatus(s: Station) {
  ...
  if (s.faultChargers > 1)
    return {
      label: "Critical",
      cls: "bg-[oklch(0.65_0.22_25)/12] text-[oklch(0.65_0.22_25)] border-[oklch(0.65_0.22_25)/30]",
    };
  if (s.faultChargers > 0 || s.uptime < 90)
    return {
      label: "Warning",
      cls: "bg-[oklch(0.72_0.16_75)/12] text-[oklch(0.72_0.16_75)] border-[oklch(0.72_0.16_75)/30]",
    };
  return {
    label: "Healthy",
    cls: "bg-[oklch(0.65_0.18_145)/12] text-[oklch(0.65_0.18_145)] border-[oklch(0.65_0.18_145)/30]",
  };
}
...
export function StationListView({
  ...
  const sorted = [...stations].sort((a, b) => {
    ...
    if (sortKey === "load") {
      va = a.currentLoadKW / a.powerCapacityKW;
      vb = b.currentLoadKW / b.powerCapacityKW;
    }
    if (sortKey === "uptime") {
      va = a.uptime;
      vb = b.uptime;
            ...
            <tr>
              {[
                ...
                <th
                  key={`${label}-${key}`}
                  className={cn(
                    "bg-muted/40 px-4 py-2.5 text-left first:rounded-tl-xl last:rounded-tr-xl border-b border-border",
                    cls,
                  )}
                >
                  <SortHeader
          ...
          <tbody>
            {sorted.map((station, idx) => {
              ...
              const utilPct = Math.round(
                (station.currentLoadKW / station.powerCapacityKW) * 100,
              );
              ...
              return (
                ...
                >
                  ...
                  <td
                    className={cn(
                      "px-4 py-3 hidden sm:table-cell min-w-[120px]",
                      !isLast && "border-b border-border",
                    )}
                  >
